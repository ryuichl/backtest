"""
00631L 回測：以 0050 200 日均線作為進出場訊號
策略比較：
  1. Buy & Hold 00631L
  2. 0050 跌破 200MA 連續3天 → 賣出換現金，漲回連續3天 → 買回
  3. 0050 跌破 200MA 連續3天 → 賣出換 0050，漲回連續3天 → 買回 00631L
"""
...
# ── 下載資料 ──────────────────────────────────────────────
print("正在下載 0050.TW、00631L.TW 歷史資料...")
...
# 計算 0050 200 日均線
df["TW50_MA200"] = df["TW50_Close"].rolling(window=200).mean()
df.dropna(inplace=True)

# 0050 是否在 200MA 之上
df["above_ma"] = (df["TW50_Close"] > df["TW50_MA200"]).astype(int)
...
# ── 輸出績效表格 ──────────────────────────────────────────
print("=" * 80)
print("         00631L 回測結果：0050 200 日均線策略 (連續3天確認)")
print("=" * 80)
